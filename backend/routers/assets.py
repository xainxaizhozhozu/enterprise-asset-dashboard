...
async def create_asset(req: AssetCreateRequest, session: AsyncSession = Depends(get_session)):
    asset = Asset(
        name=req.name,
...
@router.put("/{asset_id}")
async def update_asset(asset_id: int, req: AssetUpdateRequest, session: AsyncSession = Depends(get_session)):
    result = await session.execute(select(Asset).where(Asset.id == asset_id))
    asset = result.scalars().first()
    if not asset:
        raise HTTPException(status_code=404, detail="资产不存在")
